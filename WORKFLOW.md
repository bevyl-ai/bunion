...
- **Converge — never loop.** The factory must not spin a ticket in circles. Keep a running **`### Attempts`** list in the workpad — append ONE line each time a phase runs: `<phase> <verdict> — tested <preview-url>@<head-sha> — <one-line reason>`. BEFORE you act, read it: if this ticket has already failed YOUR phase the same way (same defect, same missing proof, same blocker), do NOT repeat the lap — route `Needs human` with `Verdict: NEEDS HUMAN — looping on <X>; a person must decide`. A pair of states traded 3+ times is a stalemate, not progress: escalate, don't re-run.
- **Record what you tested** (the #1 cause of loops). Any verdict that depends on a preview/deployment MUST name the exact **preview URL + the PR head-commit SHA** you tested against, in both the Attempts line and the Verdict. The most common false-fail is testing a **stale preview** — its deployed commit ≠ the PR head — which makes a correct fix look broken and starts an infinite bounce. Resolve the head-commit branch preview (Preview Verification / the PR's current preview), never a guessed or prod/base alias; if the deployment SHA doesn't match the PR head, that is `BLOCKED — stale preview`, NEVER a FAIL. Likewise, if a verdict you'd give demands proof that's environmentally impossible to produce, route `BLOCKED` (say what's needed), not a failure that sends a sound fix back to loop.
