...
- **Never hand-poll — use the `wait` tool.** To wait for anything async — CI checks, a code review to land, a preview deploy to come up — call `wait`: it polls host-side and spends ZERO of your tokens. `{ for: "checks" }` waits for the PR's CI and returns the failing checks; `{ for: "command", command, until: "stdout_matches", pattern }` polls anything else until its output matches. Looping `gh pr checks` / `gh api …` + `sleep` by hand burns tokens for nothing — never do it.
...
4. `commit`, then `push` (open/update the PR, ensure the `bunion` label, attach the PR URL to the issue). Then **`wait` for CI (`{ for: "checks" }`)** — it blocks token-free until checks finish and returns the failing checks; if red, fix and push again. Do NOT loop `gh pr checks` + `sleep`.
5. **Code-review gate (stupify) — the build phase is NOT done until stupify approves the latest code.** stupify reviews every push: a PR review from `exe-dev-github-integration[bot]`, tagged `<!-- stupify:<commit-sha> -->` for the commit it reviewed. **It approves with a short positive verdict ending in `✅`** — the wording varies (`LGTM ✅`, `nice, all fixed ✅`, `looks good ✅`), so match the **`✅` approval**, NOT the literal word "LGTM". If instead it describes specific problems (no `✅` verdict — e.g. `oof, this blocks…`, `one small drift trap 👇`), that's changes-requested. Loop:
   ...
   - Find stupify's review whose `stupify:<sha>` matches `head` — **or the latest real CODE commit** when the only commits after it are trivial `[skip ci]` / `chore(pr): reset branch artifacts` commits. Those don't change code, so stupify won't re-review them (`nothing new — staying silent`) and its `✅` carries forward. **No review on the latest code commit yet → `wait` for it** (`{ for: "command", command: "gh api repos/$REPO/pulls/<N>/reviews", until: "stdout_matches", pattern: "stupify:<head>", timeout_seconds: 300 }`, substituting the real `head` sha) — block token-free until stupify reviews `head`; don't hand-poll.
   ...
   - **If stupify flakes** — the `wait` above times out (~5 min) with no review of the latest code commit (it's slow, backlogged, or down) — **do NOT block the pipeline.** Note in the workpad that you proceeded without a stupify `✅` (reviewer unavailable), then move it to the QA stage anyway. A flaky reviewer must never wedge the build stage — your QA pass + the human merge are still gates. (Only genuine *changes-requested* keep you in build; absence of a review does not.)
6. Once stupify has approved (`✅`) the latest code — or has flaked per the fallback above — CI checks are green, and the acceptance criteria are met → move to `QA Requested` and verify it yourself in the QA stage.
