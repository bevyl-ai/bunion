  ...
  const done = (async (): Promise<AgentOutcome> => {
    ...
    try {
      ...
      for (let turn = 1; ; turn++) {
        ...
        if (!isActive(cfg, current.state)) break // reached a handoff state (Ready to merge / Needs Engineer / Done) — this ticket is done
